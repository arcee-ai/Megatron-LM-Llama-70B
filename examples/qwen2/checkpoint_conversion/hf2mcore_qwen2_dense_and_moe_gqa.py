...
def check_hf_mg_forward(hfmodel, mgmodel, mgargs):
    ...
    def print_input_hook(module, args, kwargs, layer_idx, mode):
        ...
        if frame == 'hf':
            #if "_proj" in mode:
            #    print(f"!!! print_input_hook HF input mode={mode} input={args[0].shape} {args[0]}")
            ...
            #if "qkv_proj_in" in mode:
            #    print(f"!!! print_input_hook MG input mode={mode} input={args[0].shape} {args[0]}")
    ...
    def print_output_hook(module, args, kwargs, output, layer_idx, mode):
        ...
        elif mode in ['hf-o_proj_out', 'hf-q_proj_out', 'hf-k_proj_out', 'hf-v_proj_out']:
    ...
    for idx, layer in enumerate(hfmodel.model.layers):
        ...
        print(f"q proj {layer.self_attn.k_proj.weight.shape}")
        print(f"q proj {layer.self_attn.v_proj.weight.shape}")
        layer.self_attn.q_proj.register_forward_pre_hook(partial(print_input_hook, layer_idx=idx, mode='hf-q_proj_in'),
                                                         with_kwargs=True)
        layer.self_attn.k_proj.register_forward_pre_hook(partial(print_input_hook, layer_idx=idx, mode='hf-k_proj_in'),
                                                         with_kwargs=True)
        layer.self_attn.v_proj.register_forward_pre_hook(partial(print_input_hook, layer_idx=idx, mode='hf-v_proj_in'),
                                                         with_kwargs=True)
        """
        ...
        """
        layer.self_attn.k_proj.register_forward_hook(partial(print_output_hook, layer_idx=idx, mode='hf-k_proj_out'),
                                                     with_kwargs=True)
        layer.self_attn.v_proj.register_forward_hook(partial(print_output_hook, layer_idx=idx, mode='hf-v_proj_out'),
                                                     with_kwargs=True)
    ...
    for idx, layer in enumerate(mgmodel.decoder.layers):
        ...
        #print(f"qMG qkv proj {layer.self_attention.linear_qkv.weight.shape}")
        #layer.self_attention.linear_qkv.register_forward_pre_hook(partial(print_input_hook, layer_idx=idx, mode='mg-qkv_proj_in'),
        #                                                 with_kwargs=True)
    ...
    for idx, (hfh, mgh) in enumerate(zip(hf_hiddens, mg_hiddens)):        
        ...
        for k, hfv in hfh.items():
            if k in mgh:
                ...
                hfv = hfv.cpu()
                print(f'layer:{idx}, {k}, size={hfv.shape} value={hfv}')
